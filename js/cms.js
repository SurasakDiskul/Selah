...
(function () {
  ...
  const STORAGE_KEY = 'selah_cms_data';
  const ADMIN_KEY = 'selah_admin_logged_in';
  ...
  const imgEls = document.querySelectorAll('[data-cms-img]');
  const bgEls = document.querySelectorAll('[data-cms-bg]');
  const totalEditable = textEls.length + imgEls.length + bgEls.length;
  ...
  style.textContent = `
    ...
    .cms-banner-badge {
      background: #ffd700; color: #1a1a2e; font-weight: 900; font-size: .65rem;
    ...
    @keyframes cmsPulse {
      ...
      50% { box-shadow: 0 0 12px 4px rgba(255,215,0,.25); }
    }
    .cms-banner-info {
    ...
    .cms-stat {
      ...
      font-size: .7rem; color: rgba(255,255,255,.8); white-space: nowrap;
    }
    .cms-stat b { color: #ffd700; }
    .cms-banner-actions { display: flex; gap: 8px; flex-shrink: 0; }
    .cms-tb-btn {
      background: rgba(255,255,255,.12); color: #fff; border: 1px solid rgba(255,255,255,.2);
      padding: 6px 14px; font-size: .72rem; font-weight: 700; letter-spacing: 1px;
      cursor: pointer; text-decoration: none; border-radius: 5px;
      transition: all .2s;
    }
    ...
    /* ── Toggle button (when banner collapsed) ── */
    #cms-toggle {
    ...
    body.cms-active [data-cms-img]:hover,
    body.cms-active [data-cms-bg]:hover {
    ...
    body.cms-active [data-cms]::after,
    body.cms-active [data-cms-img]::after,
    body.cms-active [data-cms-bg]::after {
      content: attr(data-cms-label);
      position: absolute; top: -8px; right: -4px; z-index: 100;
    ...
    body.cms-active [data-cms]:hover::after,
    body.cms-active [data-cms-img]:hover::after,
    body.cms-active [data-cms-bg]:hover::after {
      opacity: 1;
    }
    ...
    .cms-edit-overlay {
      ...
      animation: cmsFadeIn .2s ease;
    }
    @keyframes cmsFadeIn { from { opacity: 0; } to { opacity: 1; } }
    .cms-edit-box {
      background: #fff; border-radius: 14px; padding: 28px; width: 90%; max-width: 560px;
      box-shadow: 0 20px 60px rgba(0,0,0,.4); max-height: 80vh; overflow-y: auto;
      animation: cmsSlideUp .25s ease;
    }
    @keyframes cmsSlideUp { from { transform: translateY(20px); opacity: 0; } to { transform: none; opacity: 1; } }
    .cms-edit-box h3 {
      margin: 0 0 4px; font-size: 1rem; color: #1a2b5e;
      font-family: 'Barlow Condensed', sans-serif; letter-spacing: 1px; font-weight: 800;
    }
    .cms-edit-type {
      font-size: .7rem; color: #999; letter-spacing: 1px; margin-bottom: 16px;
      display: flex; align-items: center; gap: 6px;
    }
    .cms-edit-type span {
      background: #ffd700; color: #1a1a2e; padding: 1px 8px; border-radius: 3px;
      font-weight: 800; font-size: .6rem;
    }
    .cms-edit-box textarea {
      width: 100%; min-height: 120px; border: 2px solid #e0e0e0; padding: 12px;
      font-size: .9rem; font-family: inherit; border-radius: 8px; resize: vertical;
      transition: border-color .2s;
    }
    .cms-edit-box textarea:focus { outline: none; border-color: #1a2b5e; }
    .cms-edit-box input[type=text] {
      width: 100%; border: 2px solid #e0e0e0; padding: 12px;
      font-size: .9rem; font-family: inherit; border-radius: 8px;
      transition: border-color .2s;
    }
    .cms-edit-box input[type=text]:focus { outline: none; border-color: #1a2b5e; }
    .cms-edit-box .cms-preview {
      margin: 12px 0; max-width: 100%; max-height: 200px; object-fit: contain;
    ...
    .cms-edit-btns button {
      padding: 10px 24px; font-weight: 700; font-size: .82rem; border: none;
      border-radius: 8px; cursor: pointer; letter-spacing: 1px; transition: all .2s;
    }
    .cms-btn-save { background: #1a2b5e; color: #fff; }
    .cms-btn-save:hover { background: #2a3b7e; transform: translateY(-1px); }
    .cms-btn-cancel { background: #eee; color: #333; }
    .cms-btn-cancel:hover { background: #ddd; }
    .cms-btn-reset { background: #e63030; color: #fff; margin-right: auto; }
    .cms-btn-reset:hover { background: #c02020; }

    /* ── Toast ── */
    .cms-toast {
      position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%);
      z-index: 100001; background: #1a2b5e; color: #fff;
      padding: 12px 28px; border-radius: 8px; font-size: .85rem; font-weight: 700;
      letter-spacing: 1px; box-shadow: 0 8px 24px rgba(0,0,0,.3);
      animation: cmsToastIn .3s ease, cmsToastOut .3s ease 2.2s forwards;
    }
    .cms-toast.success { background: #27ae60; }
    .cms-toast.error { background: #e63030; }
  ...
  // ── Label all editables ──
  textEls.forEach(el => el.setAttribute('data-cms-label', 'EDIT TEXT'));
  imgEls.forEach(el => el.setAttribute('data-cms-label', 'EDIT IMAGE'));
  bgEls.forEach(el => el.setAttribute('data-cms-label', 'EDIT BACKGROUND'));

  // ── Build banner ──
  document.body.classList.add('cms-active');

  const savedCount = Object.keys(getData()).length;
  ...
  toggle.id = 'cms-toggle';
  toggle.title = 'Show Admin Panel';
  toggle.textContent = 'A';
  document.body.appendChild(toggle);

  document.getElementById('cmsMinimize').addEventListener('click', () => {
  ...
  function openTextEditor(el) {
    const key = el.getAttribute('data-cms');
    const data = getData();
    const current = data[key] !== undefined ? data[key] : el.innerHTML;
    ...
    document.getElementById('cmsSave').onclick = () => {
      const val = document.getElementById('cmsEditVal').value;
      const d = getData();
      d[key] = val;
      saveData(d);
      el.innerHTML = val;
    ...
    document.getElementById('cmsCancel').onclick = () => overlay.remove();
    document.getElementById('cmsReset').onclick = () => {
  ...
  function openImageEditor(el, type) {
    const attr = type === 'img' ? 'data-cms-img' : 'data-cms-bg';
    const key = el.getAttribute(attr);
    const data = getData();
    const current = type === 'img'
      ? (data[key] || el.src)
      : (data[key] || el.style.backgroundImage.replace(/^url\(['"]?|['"]?\)$/g, ''));

    const label = type === 'img' ? 'IMAGE' : 'BACKGROUND';
    ...
    const urlInput = document.getElementById('cmsImgUrl');
    const preview = document.getElementById('cmsImgPreview');
    const fileInput = document.getElementById('cmsImgFile');
    ...
    fileInput.addEventListener('change', () => {
      ...
      reader.onload = e => { urlInput.value = e.target.result; preview.src = e.target.result; };
